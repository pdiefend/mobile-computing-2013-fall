...
class SGP_Sun
{
	...
	static void calcularPosicion(SGP_Timestamp t)
	{
	...
	static void calcularPosicion(double time)
	{
		...
		M = Math.toRadians(SGP_MathematicalFunctions.modulus(358.47583 + SGP_MathematicalFunctions.modulus(35999.04975*T,360.0)
			- (0.000150 + 0.0000033*T)*(T*T),360.0));
		L = Math.toRadians(SGP_MathematicalFunctions.modulus(279.69668 + SGP_MathematicalFunctions.modulus(36000.76892*T,360.0)
			+ 0.0003025*(T*T),360.0));
		...
		O = Math.toRadians(SGP_MathematicalFunctions.modulus(259.18 - 1934.142*T,360.0));
		Lsa = SGP_MathematicalFunctions.modulus(L + C - Math.toRadians(0.00569 - 0.00479*Math.sin(O)),(Math.PI*2.0));
	...
	private static void calcularLatLonAlt(double time)
	{
		...
		lon = SGP_MathematicalFunctions.modulus(thetaAux - SGP_Time.thetaG_JD(time),Math.PI*2.0);
		...
		do
		{
			...
			lat = SGP_MathematicalFunctions.acTan(pos[2] + SGP_Constants.xkmper*c*e2*Math.sin(phi),r);
		}
