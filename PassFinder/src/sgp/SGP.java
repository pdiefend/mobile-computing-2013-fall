...
class SGP {
	...
	static void SGP4(Satelite sat) {
		...
		if (sat.iflag == 1) {
			...
			sat.del1 = 1.5 * Constants.ck2 * sat.x3thm1
					/ (sat.a1 * sat.a1 * sat.betao * sat.betao2);
			sat.ao = sat.a1
					* (1.0 - sat.del1
							* (0.5 * Constants.tothrd + sat.del1
									* (1.0 + 134.0 / 81.0 * sat.del1)));
			sat.delo = 1.5 * Constants.ck2 * sat.x3thm1
			...
			sat.perige = (sat.aodp * (1.0 - sat.eo) - Constants.ae)
					* Constants.xkmper;
			if (!(sat.perige >= 156.0)) {
				sat.s4 = sat.perige - 78.0;
				if (!(sat.perige > 98.0))
					sat.s4 = 20.0;
				sat.qoms24 = Math.pow((120.0 - sat.s4) * Constants.ae
						/ Constants.xkmper, 4.0);
			...
			sat.c2 = sat.coef1
					...
					* (sat.aodp
							* (1.0 + 1.5 * sat.etasq + sat.eeta
									* (4.0 + sat.etasq)) + 0.75 * Constants.ck2
							* sat.tsi / sat.psisq * sat.x3thm1
							* (8.0 + 3.0 * sat.etasq * (8.0 + sat.etasq)));
			sat.c1 = sat.bstar * sat.c2;
			sat.sinio = Math.sin(sat.xincl);
			sat.a3ovk2 = (-Constants.xj3) / Constants.ck2
					* Math.pow(Constants.ae, 3.0);
			sat.c3 = sat.coef * sat.tsi * sat.a3ovk2 * sat.xnodp * Constants.ae
					* sat.sinio / sat.eo;
			...
			sat.xmcof = -Constants.tothrd * sat.coef * sat.bstar * Constants.ae
					/ sat.eeta;
